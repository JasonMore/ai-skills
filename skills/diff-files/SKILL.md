...
description: >
  ...
  "compare versions", or mentions file comparison and difference analysis,
  using the grepika MCP server.
author: JasonMore
...
Requires the [grepika](https://github.com/agentika-labs/grepika) MCP server configured in Copilot. Tools are exposed with the `grepika-` prefix (e.g. `grepika-diff`, `grepika-get`) — adjust the prefix if the server is configured under a different name in `~/.copilot/mcp-config.json`.

## Input

Identify the files to compare from the user's request. Expected formats:
...
2. **Get file outlines first** (optional, for large files)
   - Use `grepika-outline` to understand each file's structure
   - Identify key sections to focus the comparison

3. **Run the diff**
   - Use `grepika-diff` with the two file paths
...
5. **Get context if needed**
   - Use `grepika-get` to read surrounding code for complex changes
